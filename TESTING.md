...
| 500 Server Error Page | Application must safely handle unexpected server errors without exposing stack traces. | Temporarily triggered an internal server error and accessed a broken URL to force a server failure. | Custom 500 error page displayed with no Django debug output; user sees a safe, friendly error message. | ![screenshot](documentation/defensive/500error.png) |
